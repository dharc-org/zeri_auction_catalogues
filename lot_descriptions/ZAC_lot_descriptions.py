...
from ZAC_NER import AuctionPipeline
...
def process_lot_descriptions(catalogue_id, chunks, writer=None, pipe=None):
    ...
    for chunk in chunks:
        ...
        # NER
        result = pipe.process(text, reconcile=False)
        ...
        print(f"{lot_id} | {result.author} | {result.school} | {result.period} | {result.object_type}")
...
def main():
    ...
    output_path = BASE_DIR / "lot_descriptions" / "zac_lots_ner.csv"
    fieldnames = ["lot_id", "text", "author", "school", "period", "object_type"]

    with open(output_path, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        pipe = AuctionPipeline(api_key="REMOVED")

        for catalogue_id in catalogues:
            chunks = fetch_chunks(conn, catalogue_id)
            process_lot_descriptions(catalogue_id, chunks, writer, pipe)

    conn.close()
    g.serialize(BASE_DIR / "lot_descriptions" / 'zac_lot_descriptions.trig', format='trig')
